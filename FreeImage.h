...
It is up to you to interpret these bytes correctly, according  to  the  results  of  FreeImage_GetBPP, FreeImage_GetRedMask,
...
access).
Parameter x is the pixel position in horizontal direction, 
and parameter y is the pixel position in vertical direction.
The function returns TRUE on success, and returns FALSE otherwise(e.g. for RGB[A] images).*/
...
For example when each pixel takes 32 - bits of space in the bitmap, this function returns 32. Possible bit depths are 1, 4, 8, 16, 24, 32 for
...
If the bitmap doesn¡¯t have a palette (i.e. when the pixel bit depth is greater than 8), this function returns NULL.
